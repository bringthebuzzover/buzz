...
Issued when an admin approves a brand. ``token_hash`` is the SHA-256 hex of the
raw link secret (unique) so a single click consumes a row exactly once;
``used_at`` flips from NULL to a timestamp on redemption.
"""
