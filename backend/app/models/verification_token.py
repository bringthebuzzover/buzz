...
Issued during org onboarding (architecture §3.3). ``token`` is unique so a
single click consumes a row exactly once; ``used_at`` flips from NULL to a
timestamp on redemption. Expired/used tokens are kept for audit and swept
later by the cleanup job (§10.3).
"""
