 ...
 * the bundle compiles, where a real API URL isn't needed. Railway / production
 * deploys should call `npm run build:prod` (or `predeploy`) so this guard runs.
 */
if (!process.env.REACT_APP_API_URL) {
  console.error(
    "\n✗ REACT_APP_API_URL is not set.\n" +
      ...
      "    REACT_APP_API_URL=https://api.bringthebuzzover.com npm run build:prod\n",
  );
  process.exit(1);
}
