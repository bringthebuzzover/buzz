...
| **Buzz Admin** | Local stdio [`tools/buzz-admin-mcp/`](tools/buzz-admin-mcp/): inspect allowlisted tables, patch profile/copy fields, run `/api/admin/*` actions. Defaults to production API. Column policy: [`ideas/archive/admin-mcp.md`](ideas/archive/admin-mcp.md) | Brand/org agents; impersonate; raw SQL; secrets (`password_hash`, IG token, `token_hash`, Graph user ids, `token_version`). Mutate only with explicit user OK. Never commit admin email/password |
