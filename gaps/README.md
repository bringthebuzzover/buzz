...
- **Close policy:** delete the file when fixed (git history is the archive).
- Statuses that stay on disk: `open` | `in_progress` | `deferred` | `ops` | `wontfix`.
...
status: open | in_progress | deferred | ops | wontfix
...
fix_when: |
  Acceptance criteria for deleting this file.
---
```
...
3. **Done** → delete `gaps/<id>.md`.
4. **Defer / ops / wontfix** → keep the file; change `status` only.
